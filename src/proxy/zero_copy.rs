...
// ============================================================================
// macOS/BSD 零拷贝实现（使用 sendfile）
// ============================================================================

#[cfg(target_os = "macos")]
use libc::{off_t, c_int};

/// macOS sendfile() 系统调用包装
///
/// macOS 的 sendfile 比 Linux splice() 更简单，直接两个 fd 之间传输。
/// 注意：macOS 的 sendfile 主要用于文件到 socket，但也可以用于 socket-to-socket（需要内核支持）。
#[cfg(target_os = "macos")]
#[inline]
unsafe fn sendfile_syscall(
    fd: i32,
    s: i32,
    off: off_t,
    len: *mut off_t,
    hdtr: *mut libc::sf_hdtr,
    flags: c_int,
) -> i32 {
    libc::sendfile(fd, s, off, len, hdtr, flags)
}

/// macOS 平台：零拷贝双向转发
///
/// 对于 socket-to-socket 转发，macOS 的 sendfile 有限制。
/// 我们使用优化的 bulk_copy（大缓冲区 + 零拷贝友好策略）。
/// 未来可以改用 `kqueue` + `kevent` 实现事件驱动的高性能转发。
#[cfg(target_os = "macos")]
pub async fn zero_copy_bidirectional(client: TcpStream, server: TcpStream) -> Result<(u64, u64)> {
    debug_println!("🚀 macOS: 使用优化的 bulk_copy (128KB)");
