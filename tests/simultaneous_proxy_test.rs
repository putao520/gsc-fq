...
use anyhow::Result;
use gsc_fq::config::loader::{ConfigFile, ReverseProxyServerSection, ServerSection};
use std::net::{IpAddr, Ipv4Addr};
use std::time::Duration;
use support::{wait_for_port_ready, PingPongServer, LOCALHOST};
...
async fn test_simultaneous_forward_and_reverse_proxy() -> Result<()> {
    ...
    println!(
        "   - 正向代理: {} → 127.0.0.1:{}",
        forward_proxy_port, forward_target_port
    );
    println!(
        "   - 反向代理服务器模式: 控制端口 {}, 服务端口 {}",
        reverse_control_port, reverse_proxy_port
    );
    println!(
        "   - 反向代理转发: {} → 127.0.0.1:{}",
        reverse_proxy_port, reverse_target_port
    );
    ...
    let service_handle = tokio::spawn(async move {
        ...
        let mut reverse_server =
            gsc_fq::reverse_proxy::ReverseProxyServer::new(bind_ip, reverse_control_port);
        ...
        // 启动反向代理客户端 (CRITICAL FIX: This was missing!)
        // 我们需要配置客户端指向我们的 server
        let client_config = ConfigFile {
            // Re-use parts of config or creaet new one.
            // IMPORTANT: The original `config` has `reverse_proxies` which are client configs.
            // We need to pass THAT to the client.
            reverse_proxy_client: Some(gsc_fq::config::loader::ReverseProxyClientSection {
                server: format!("127.0.0.1:{}", reverse_control_port),
        ...
        // 注意：ReverseProxyClient 需要连接到 Server
        // 给 Server 一点时间启动
        tokio::time::sleep(Duration::from_millis(500)).await;

        let server_addr = format!("127.0.0.1:{}", reverse_control_port)
            .parse()
            .unwrap();
        let mut client = gsc_fq::reverse_proxy::ReverseProxyClient::new(server_addr, client_config);
        ...
        // 等待所有服务
        let _ = tokio::join!(reverse_handle, forward_handle, client_handle);
    });
...
async fn test_multiple_proxy_configurations() -> Result<()> {
    ...
    println!(
        "   正向代理: {:?} → {:?}",
        forward_ports,
        target_ports[0..2].to_vec()
    );
    println!(
        "   反向代理: {:?} → {:?}",
        reverse_ports,
        target_ports[1..3].to_vec()
    );
    ...
    let mut config = ConfigFile {
        ...
        proxies: forward_ports
            .iter()
            .zip(target_ports[0..2].iter())
            .map(
                |(&local_port, &target_port)| gsc_fq::config::loader::ProxySection {
        ...
        reverse_proxies: reverse_ports
            .iter()
            .zip(target_ports[1..3].iter())
            .map(
                |(&server_port, &target_port)| gsc_fq::config::loader::ReverseProxySection {
